...
for iso in ["iso", "antiiso"]:
    for channel in ["mu", "ele"]:
        #data_files[iso+channel] = get_data_files(iso, channel)
...
for iso in ["iso", "antiiso"]:
    for channel in ["mu", "ele"]:
        for dataset, fileset in data_files[iso+channel].items():
            #if dataset != "TTJets_SemiLept": continue
            #if not ("Single" in dataset and iso == "iso"): continue
            i = 0
            print
            print dataset
            ...
            for (base_file, added_file) in fileset:
                print i, base_file
                bf_name = "/tmp/andres/varhistos_%s_%s_%s_%d.sh" % (dataset, channel, iso, i)
                batch_outfile = open(bf_name, "w")
                batch_outfile.write("#!/bin/bash\n")
                batch_outfile.write("source $STPOL_DIR/setenv.sh\n")
                batch_outfile.write("python $STPOL_DIR/src/qcd_ntuples/histos.py " +channel+" "+dataset+ " " +str(i)+" "+iso+" " +base_file+" " + added_file + "\n")
                batch_outfile.close()
