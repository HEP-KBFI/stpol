...
def plot_comparisons(filenames, histos, title):
    ...
    for syst in systs:
        for var in variations:
            ...
            mytitle = "2J1T, %s %s smoothing, %s channel, BDT > %s" % (syst, var, title.split("_")[0], title.split("_")[1][:4])
...
def plot_templates(histo1, histo2, title, desc):
    ...
    histo1.SetAxisRange(0, histo1.GetMaximum()*1.2, "Y")
    ...
    leg.AddEntry(histo1,"smoothed","l")
    leg.AddEntry(histo2,"not smoothed","pl")
    leg.Draw()

    outname = "comparison_plots/"+desc
...
if __name__ == "__main__":
    ...
    for channel in ["mu", "ele"]:
        for cut in ["0.30000", "0.45000", "0.60000"]:
            files = {}
            files["smoothed"] = "/home/andres/single_top/stpol_pdf/results/hists/Mar23/merged/%s/%s/cos_theta_lj.root" % (cut, channel)
            files["not smoothed"] = "/home/andres/single_top/stpol_pdf/results/hists/Mar9/merged/%s/%s/cos_theta_lj.root" % (cut, channel)
