...
def get_weights_nnpdf(dataset, thispdf, channel, filecounter, filename, dont_skim = False):
    maxscale = 200
    minscale = 170
    maxid = 0
    minid = 0
    ...
    for p, lst in pdfs.items():
        ...
        #print file_list
        events[p] = Events(filename)
        #for l in lst:        
        handles[thispdf] = Handle ('std::vector<float>')
        weight_sum[thispdf] = [0] * sizes[thispdf]
    ...
    for p in pdfs:
        ...
        for event in events[p]:
            ...
            for i in range(len(pdfs[p])):
                if not pdfs[p][i] == thispdf: continue
...
if __name__ == "__main__":
    ...
    (weights, wsum, others) = get_weights("T_t_ToLeptons", "NNPDF23nloas0118LHgrid", "mu", 0)
    print wsum, weights
