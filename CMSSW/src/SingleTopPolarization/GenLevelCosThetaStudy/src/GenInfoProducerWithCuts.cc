// -*- C++ -*-
//
// Package:    GenInfoProducerWithCuts
// Class:      GenInfoProducerWithCuts
//
/**\class GenInfoProducerWithCuts GenInfoProducerWithCuts.cc GenInfoAnalyzer/GenInfoProducerWithCuts/src/GenInfoProducerWithCuts.cc
...
class GenInfoProducerWithCuts : public edm::EDFilter
{
public:
    explicit GenInfoProducerWithCuts(const edm::ParameterSet &);
    ~GenInfoProducerWithCuts();
...
GenInfoProducerWithCuts::GenInfoProducerWithCuts(const edm::ParameterSet &iConfig)
    :
    jet_lepton_dr(0.5),
    lepton_pt_low(26)
{
...
GenInfoProducerWithCuts::~GenInfoProducerWithCuts()
{
    delete rng;
}
...
bool
GenInfoProducerWithCuts::filter(edm::Event &iEvent, const edm::EventSetup &iSetup)
{
    using namespace edm;
    using namespace std;
    ...
    for (size_t i = 0; i < genParticles->size(); ++i)
    {
        ...
        if (abs(p.pdgId()) == 13 && p.status() == 1 && p.pt() > lepton_pt_low && abs(p.eta()) < 2.1 )
        {
...
void
GenInfoProducerWithCuts::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void
GenInfoProducerWithCuts::endJob()
{
}

// ------------ method called when starting to processes a run  ------------
bool
GenInfoProducerWithCuts::beginRun(edm::Run &, edm::EventSetup const &)
{
    return true;
}
...
bool
GenInfoProducerWithCuts::endRun(edm::Run &, edm::EventSetup const &)
{
    return true;
}
...
bool
GenInfoProducerWithCuts::beginLuminosityBlock(edm::LuminosityBlock &, edm::EventSetup const &)
{
    return true;
}
...
bool
GenInfoProducerWithCuts::endLuminosityBlock(edm::LuminosityBlock &, edm::EventSetup const &)
{
    return true;
}
...
void
GenInfoProducerWithCuts::fillDescriptions(edm::ConfigurationDescriptions &descriptions)
{
...
DEFINE_FWK_MODULE(GenInfoProducerWithCuts);
